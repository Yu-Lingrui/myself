...
// #include </usr/local/cuda-11.3/targets/x86_64-linux/include/cuda.h>
// #include </usr/local/cuda-11.3/targets/x86_64-linux/include/cuda_runtime.h>
// #include </usr/local/cuda-11.3/targets/x86_64-linux/include/cuda_runtime_api.h>
...
void YOLOv8_pose::make_pipe(bool warmup) {
    ...
    // cudaArray *d_array;
    // CHECK(cudaMallocArray(&d_array, &channelDesc, bindings.size, bindings.dsize));
    // this->device_ptrs.push_back(d_array);
    // }
...
void YOLOv8_pose::postprocess(
    ...
    for (int i = 0; i < num_anchors; i++) {
        ...
        // 将类别改为10，5-10位为class
        auto kps_ptr = row_ptr + 10;

        //float score = *scores_ptr;
        // 类别获取
        cv::Mat scores(1, 6, CV_32FC1, scores_ptr);
        cv::Point class_id;
        ...
        if (maxClassScore > score_thres) {
            ...
            labels.push_back(class_id.x); // 压入类别
            scores.push_back(maxClassScore); // 压入分数
            kpss.push_back(kps);
        }
    }
