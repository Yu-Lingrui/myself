...
struct Configuration
{
    ...
    // --------------------------------- 通常需要根据需要修改 START -----------------------------------------
    // 3. 算法配置参数
    AlgoConfig algoConfig = {0.5}; // 默认的算法配置
    // 4. 与报警信息相关的配置
    std::string language = "en";                             // 所显示文字的默认语言
    int targetRectLineThickness = 4;                         // 目标框粗细
    std::map<std::string, std::vector<std::string> > targetRectTextMap = { {"en",{"stand","sit","crouch","prostrate_sleep","sit_sleep","lie_sleep"}}, {"zh", {"站着的人","坐着的人","蹲着的人","趴着睡的人","坐着睡的人","躺着睡的人（身体横着的)"}}};// 检测目标框顶部文字
