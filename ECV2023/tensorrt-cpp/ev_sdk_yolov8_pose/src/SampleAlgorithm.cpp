...
STATUS SampleAlgorithm::Init()
{
    ...
    mDetector->Init("/usr/local/ev_sdk/model/best.onnx", mConfig.algoConfig.thresh);
...
//    }
    ...
    for (auto &object : validTargets)
    {
        if (mConfig.drawResult)
        {
            ...
            cv::Rect rect = cv::Rect{object.x, object.y, object.w, object.h};
    ...
    jAlgoValue["target_count"] = validTargets.size();
    jAlgoValue["target_info"].resize(0);
    for (auto &obj : validTargets)
    {        
        Json::Value jTmpValue;
        Json::Value jKeyPoints;
        for(auto &k: obj.kps)
        {
            jKeyPoints["keypoints"].append(k);
        }
        jKeyPoints["score"]=obj.score;
        ...
        jTmpValue["keypoints"] = jKeyPoints;
