...
//    每行的数量m_iClassNums + 4 + 17*3
        ...
        if(*max_cls_pos > mThresh)
        {
            ...
            if( box.area() > 0)
            {
                BoxInfo box_info = { box.x, box.y, box.width, box.height, (*max_cls_pos), max_cls_pos - (prob + class_index), kps};
