...
STATUS SampleAlgorithm::Init()
{
    ...
    mDetector->Init(mConfig.algoConfig.thresh);
...
STATUS SampleAlgorithm::Process(const cv::Mat &inFrame, const char *args, JiEvent &event)
{
    ...
    std::vector<Box_pts_Info> detectedObjects;    
    std::vector<Box_pts_Info> validTargets;
    // 算法处理
...
//     SDKLOG_FIRST_N(INFO, 5) << "detected targets : " << detectedObjects.size() << " valid targets :  " << validTargets.size();
    ...
    {        
        ...
        for(int i = 0; i< 17; ++i)
        {
            coordinate.append(obj.keypoints[3*i]);
            coordinate.append(obj.keypoints[3*i+1]);
            coordinate.append(obj.keypoints[3*i+2]);
        }
        Json::Value kptValue;
        kptValue["keypoints"] = coordinate;
        kptValue["score"] = 0.77;
        jTmpValue["keypoints"] = kptValue;
