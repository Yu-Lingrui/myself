...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.interpolate(x, size=(int(sh[2] * self.scale), int(sh[3] * self.scale)), mode=self.mode,
...
class Yolo_Layers(nn.Module):
    def __init__(self, nc=80, anchors=(), ch=(), export_onnx=False):  # detection layer
        ...
        self.anchor_grid = torch.tensor(anchors).float().view(self.nl, 1, -1, 1, 1, 2)
        self.anchors = (self.anchor_grid.view(self.nl, -1, 2) / self.stride.view(-1, 1, 1))
        self.export_onnx = export_onnx  # onnx export
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training and not self.export_onnx:  # inference
                ...
                y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + self.grid[i].to(x[i].device)) * int(
                    self.stride[i].to(x[i].device))  # xy
