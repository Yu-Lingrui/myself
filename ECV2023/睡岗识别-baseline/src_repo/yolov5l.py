...
class My_YOLO(nn.Module):
    def __init__(self, num_classes, anchors=(), export_onnx=False):
        super().__init__()
        self.backbone_head = My_YOLO_backbone_head()
        self.yolo_layers = Yolo_Layers(nc=num_classes, anchors=anchors, ch=(256,512,1024), export_onnx=export_onnx)
    def forward(self, x):
