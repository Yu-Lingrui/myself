...
class PromptEncoder(nn.Module):
    ...
    def _embed_points(
        ...
        if pad:
            ...
            points = jt.contrib.concat([points, padding_point], dim=1)
            labels = jt.contrib.concat([labels, padding_label], dim=1)
    ...
    def _embed_boxes(self, boxes: jt.array) -> jt.array:
        """Embeds box prompts."""
    ...
    def _embed_masks(self, masks: jt.array) -> jt.array:
        """Embeds mask inputs."""
    ...
    def _get_batch_size(
        self,
        points: Optional[Tuple[jt.array, jt.array]],
        boxes: Optional[jt.array],
        masks: Optional[jt.array],
    ) -> int:
        """
    ...
    def execute(
        ...
        if points is not None:
            ...
            sparse_embeddings = jt.contrib.concat([sparse_embeddings, point_embeddings], dim=1)
        if boxes is not None:
            box_embeddings = self._embed_boxes(boxes)
            # sparse_embeddings = torch.cat([sparse_embeddings, box_embeddings], dim=1)
            sparse_embeddings = jt.contrib.concat([sparse_embeddings, box_embeddings], dim=1)
