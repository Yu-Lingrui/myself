...
class SamAutomaticMaskGenerator:
    ...
    def _process_batch(
        ...
        in_labels = jt.ones(in_points.shape[0], dtype=jt.int32)
        ...
        keep_mask = jt.logical_not(is_box_near_crop_edge(data["boxes"], crop_box, [0, 0, orig_w, orig_h]))
    ...
    def postprocess_small_regions(
        ...
        masks = jt.contrib.concat(new_masks, dim=0)
...
#     Returns:
#         jt.Var: int64 tensor with the indices of the elements that have been kept by NMS, sorted
#         in decreasing order of scores
#     """
#     areas = box_area(boxes)

#     # Sort boxes by scores in descending order
#     sorted_indices, _ = jt.argsort(scores, descending=True)
#     selected_indices = []
...
#         intersection_x1 = jt.maximum(current_box[0], boxes[sorted_indices][:, 0])
#         intersection_y1 = jt.maximum(current_box[1], boxes[sorted_indices][:, 1])
#         intersection_x2 = jt.minimum(current_box[2], boxes[sorted_indices][:, 2])
#         intersection_y2 = jt.minimum(current_box[3], boxes[sorted_indices][:, 3])

#         intersection_widths = jt.maximum(intersection_x2 - intersection_x1, 0)
#         intersection_heights = jt.maximum(intersection_y2 - intersection_y1, 0)
#         intersection_areas = intersection_widths * intersection_heights

#         union_areas = current_area + areas[sorted_indices] - intersection_areas
#         ious = intersection_areas / union_areas

#         # Filter out boxes with IoU greater than the threshold
#         indices_to_keep = jt.where(ious <= iou_threshold)[0]
#         sorted_indices = sorted_indices[indices_to_keep]

#     return jt.array(selected_indices, dtype='int64')
...
def batched_nms(
    ...
    keep = _batched_nms_vanilla(boxes, scores, idxs, iou_threshold)
    return keep
...
def _batched_nms_vanilla(
    ...
    _, sorted_indices = scores[keep_indices].sort(descending=True)
