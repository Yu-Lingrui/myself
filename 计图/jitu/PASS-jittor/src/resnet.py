...
class C3(nn.Module):
    ...
    def execute(self, x):
        return self.cv3(concat((self.m(self.cv1(x)), self.cv2(x)), 1))
...
class SPPF(nn.Module):
    ...
    def execute(self, x):
        ...
        y1 = self.m(x)
        y2 = self.m(y1)
        return self.cv2(concat((x, y1, y2, self.m(y2)), 1))
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.bn1 = C3(width, width)
        # self.bn1 = norm_layer(width)
        self.conv2 = conv3x3(width, width, stride, groups, dilation)
        self.bn2 = C3(width, width)
        # self.bn2 = norm_layer(width)
        self.conv3 = conv1x1(width, planes * self.expansion)
        self.bn3 = C3(planes * self.expansion, planes * self.expansion)
        # self.bn3 = norm_layer(planes * self.expansion)
        self.bn4 = SPPF(planes * self.expansion, planes * self.expansion)
        self.relu = nn.ReLU()
        self.downsample = downsample
        self.stride = stride
