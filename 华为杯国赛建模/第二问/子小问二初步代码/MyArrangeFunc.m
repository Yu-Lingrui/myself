...
if (p1<5)
    ...
    if p1 == 3
        [indices,result]=rpfun(x, y + d, w, h - d, D, rm, indices, result,dc,dgw);
    elseif p1 == 2
        [indices,result]=rpfun(x + omega, y, w - omega, h, D, rm, indices, result,0,0);
    elseif p1 >=4 && p1<5
        min_w = inf;
        min_h = inf;
        for idx=indices
        ...
        elseif h - d < min_h
            [indices,result]=rpfun(x + omega, y, w - omega, h, D, rm, indices, result,0,0);
        elseif omega < min_w
            [indices,result]=rpfun(x + omega, y, w - omega, d, D, rm, indices, result,dc,dgw);
            [indices,result]=rpfun(x, y + d, w, h - d, D, rm, indices, result,dc,dgw);
        else
            [indices,result]=rpfun(x, y + d, omega, h - d, D, rm, indices, result,dc,dgw);
            [indices,result]=rpfun(x + omega, y, w - omega, h, D, rm, indices, result,0,0);
        end
    end
end
